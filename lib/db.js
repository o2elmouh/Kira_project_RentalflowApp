...
function inferRepairType(label = '') {
  ...
  if (/vidange|huile|filtre|courroie|pneu|freins?|batterie|amortisseur|embrayage|bougie/.test(l)) return 'maintenance'
  if (/inspection|contrôle|visite|révision technique/.test(l)) return 'inspection'
...
function repairToDb(r) {
  return {
    ...
    repair_type:       r.type        || r.repair_type || inferRepairType(r.description || r.label),
