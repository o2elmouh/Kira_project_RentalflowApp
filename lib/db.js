...
// Module-scope cache: agency_id is stable for the lifetime of an auth session.
// Resolving it cost two Supabase round-trips (auth.getUser + SELECT profiles)
// that fired on every db.js call (~20 functions), adding 600-1000 ms to flows
// like the new-rental vehicle picker. We cache per-user and invalidate on
// SIGNED_OUT / USER_UPDATED so a session swap can't leak across tenants.
let _agencyIdCache = { userId: null, agencyId: null, promise: null }

supabase.auth.onAuthStateChange((event) => {
  if (event === 'SIGNED_OUT' || event === 'USER_UPDATED') {
    _agencyIdCache = { userId: null, agencyId: null, promise: null }
  }
})

export async function getAgencyId() {
  // Use getSession() (synchronous-ish, reads from localStorage) instead of
  // getUser() which always hits /auth/v1/user (200-500 ms per call).
  const { data } = await supabase.auth.getSession()
  const userId = data?.session?.user?.id ?? null
  if (!userId) return null
  if (_agencyIdCache.userId === userId && _agencyIdCache.agencyId !== null) {
    return _agencyIdCache.agencyId
  }
  if (_agencyIdCache.userId === userId && _agencyIdCache.promise) {
    return _agencyIdCache.promise
  }
  const promise = (async () => {
    ...
    _agencyIdCache = { userId, agencyId, promise: null }
    return agencyId
  })()
  _agencyIdCache = { userId, agencyId: null, promise }
  return promise
}
...
// ─── Clients ─────────────────────────────────────────────────
// Moved to server/routes/clients.js (Phase 5). Frontend now imports
// { api } from '../lib/api' and calls api.getClients / saveClient /
// patchClient / deleteClient / getClient. The mappers (clientToDb /
// clientFromDb) live in server/routes/clients.js so encrypt-on-write
// and decrypt-on-read stay server-side.
...
export async function getFleet() {
  // v1.15.1: reverted to sbSelect (which uses select('*')) after v1.15.0's
  // explicit column list broke production. PostgREST fails the entire query
  // when a listed column doesn't exist, but select('*') tolerantly returns
  // whatever columns the table actually has. The 'category' column we listed
  // is a frontend-only field that vehicleFromDb defaults — it doesn't exist
  // in the live schema. Schema drift between environments + future feature
  // columns make explicit lists fragile; the byte savings weren't worth it.
...
export async function getContracts() {
  const agencyId = await getAgencyId()
  if (!agencyId) return []
  // v1.15.1: reverted to sbSelect (select('*')) — see getFleet comment above.
  // Live contracts schema is missing several columns we explicitly listed
  // (client_name, vehicle_name, signature_status, signed_at, signed_pdf_path,
  // finalized_at, prolongation_target_contract_id). PostgREST fails the whole
  // query on any missing column; select('*') tolerantly returns what exists.
...
/**
 * Returns active contracts on `vehicleId` whose [pickup_date, return_date]
 * overlaps with [startDate, endDate], excluding `excludeContractId` (so a
 * contract doesn't flag itself as a conflict).
 *
 * Same overlap predicate as the `get_available_vehicles` RPC:
 *   pickup_date < endDate AND return_date > startDate
 * — half-open intervals (a contract ending the same day a new one starts is
 * NOT a conflict).
 *
 * Used by LeadModal to detect whether the original car for a prolongation
 * request is already booked for the extension window — if it is, the agent
 * gets an amber warning + a smart-quote fallback path to propose another car.
 */
export async function findVehicleConflicts(vehicleId, startDate, endDate, excludeContractId) {
  ...
  const agencyId = await getAgencyId()
  if (!agencyId) return []
  // v1.15.2: dropped `client_name` from the SELECT — that column doesn't
  // exist on the contracts schema (never has). PostgREST was failing the
  // entire query with 42703, which silently broke the LeadModal conflict
  // warning since v1.14.20. Callers only ever read .startDate/.endDate
  // anyway (see LeadModal — vehicle label comes from targetContract, not
  // from the conflict row).
  let q = supabase
    .from('contracts')
    .select('id, contract_number, pickup_date, return_date, vehicle_id')
    .eq('agency_id', agencyId)
...
export async function getInvoices() {
  // v1.15.1: reverted to sbSelect — same schema-drift risk as getFleet/getContracts.
  // The narrowed list worked on prod (verified) but the consistency win of using
  // sbSelect everywhere prevents the same trap if invoices schema diverges later.
...
function inferRepairType(label = '') {
  const l = label.toLowerCase()
  if (/vidange|huile|filtre|courroie|pneu|freins?|batterie|amortisseur|embrayage|bougie|suspension|climatisation|électronique|revision|révision/.test(l)) return 'maintenance'
  if (/inspection|contrôle|visite|technique/.test(l)) return 'inspection'
  if (/répar|accident|carrosserie|pare|vitres?|choc|dommage/.test(l)) return 'repair'
  return 'other'
}
...
export async function addRepair(vehicleId, repair) {
  ...
  const { error } = await supabase.from('repairs').insert({
    ...
    agency_id:   agencyId,
    repair_type: inferRepairType(repair.label),
...
function repairToDb(r) {
  return {
    ...
    repair_type:       inferRepairType(r.type || r.repair_type || r.description || r.label),
    description:       r.description || r.label || r.type || null,
...
export async function getTCO(vehicleId) {
  ...
  const { data, error } = await supabase.from('repairs').select('cost').eq('vehicle_id', vehicleId)
  if (error) throw error
  const rows = data || []
  const totalExpense = rows.reduce((s, r) => s + (Number(r.cost) || 0), 0)
  return { totalExpense, totalInsurance: 0, totalFranchise: 0, totalRecovered: 0, netTCO: totalExpense }
}
...
// getClient(id) moved to api.getClient (server/routes/clients.js, Phase 5).
...
export async function initDefaultAccounts() {
  // no-op for Supabase — seeded by the `agencies_seed_accounts` trigger
  // (migration 20260608_accounting_schema_fix.sql).
}
...
export async function saveTransaction(tx) {
  ...
  Object.keys(dbRow).forEach(k => dbRow[k] === undefined && delete dbRow[k])
  // Auto-generate a human-readable reference if missing (TXN-YYYYMMDD-NNNN).
  if (!dbRow.reference) {
    ...
    dbRow.reference = `TXN-${stamp}-${suffix.toUpperCase()}`
  }
...
export async function getJournalEntries() {
  const rows = await sbSelect('journal_entries')
...
export async function saveJournalEntry(entry) {
  const dbRow = journalEntryToDb(entry)
  Object.keys(dbRow).forEach(k => dbRow[k] === undefined && delete dbRow[k])
  const saved = await sbUpsert('journal_entries', dbRow)
  return journalEntryFromDb(saved)
}
...
export async function getDeposits() {
  const rows = await sbSelect('deposits')
  return rows.map(depositFromDb)
}
...
export async function getDepositByContract(contractId) {
  ...
  return rows[0] ? depositFromDb(rows[0]) : null
}
