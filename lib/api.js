...
export const api = {
  ...
  // Clients CRUD (Phase 5 — backend-only PII layer).
  // Frontend is NOT yet switched to these (that's v1.10.7). The endpoints are
  // live in v1.10.6 so they can be smoke-tested before the cut-over.
  getClientsApi: ()              => request('GET',    '/clients'),
  getClientApi:  (id)            => request('GET',    `/clients/${id}`),
  saveClientApi: (client)        => request('POST',   '/clients', client),
  patchClientApi:(id, patch)     => request('PATCH',  `/clients/${id}`, patch),
  deleteClientApi:(id)           => request('DELETE', `/clients/${id}`),
