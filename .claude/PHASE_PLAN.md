...
| **5b** — Frontend switch (still flag OFF) | ⏳ NOT STARTED | TODO: replace `lib/db.js` client functions with `lib/api.js` calls across `pages/Clients.jsx`, `pages/Restitution.jsx`, `pages/Basket.jsx`, `pages/rental/ScanStep.jsx`, `pages/rental/ContractStep.jsx`. Plaintext column reads still authoritative. |
